...
export class TranslationViewModelService {
  ...
  readonly isLoading: Observable<boolean> = this.state.pipe(
    map((s: TranslationViewModel): boolean => s.isLoading));
  readonly translations: Observable<Translation[]> = this.state.pipe(
    map((s: TranslationViewModel): Translation[] => s.translations));
  readonly languages: Observable<Language[]> = this.state.pipe(
    map((s: TranslationViewModel): Language[] => s.languages));
  readonly compatibleCountries: Observable<Country[]> = this.state.pipe(
    map((s: TranslationViewModel): Country[] => s.compatibleCountries));
  readonly nonCompatibleCountries: Observable<Country[]> = this.state.pipe(
    map((s: TranslationViewModel): Country[] => s.nonCompatibleCountries));
  ...
  processTranslation(formValues: TranslationForm): Observable<void> {
    ...
    try {
      const request: TranslationRequest = this.translationDomainService.createTranslationRequest(
        formValues.query,
        formValues.langCode,
        formValues.countryCode
      );
      return this.translationApplicationService.processTranslationRequest(request).pipe(
        tap((result: TranslationProcessResult) => {
          this.updateState({
            isLoading: false,
